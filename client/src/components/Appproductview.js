...
const Appproductview = ({umail}) => {
    ...
    function l () {
        ...
        client.post("productview",data).then((res)=>{
    ...
    function myCart(){
      ...
      client.post("newcart",data).then((res)=>{
        if (res.data === true){
          handleShowc()
        }
      }).catch((error)=>{
      ...
      <Row>
        <Col>
            <Card>
                <Carousel>
                <Carousel.Item>
                <img className="d-block w-100" src={pimg} alt="Second slide" style={{height:"500px", width:"700px"}}/>
                </Carousel.Item>
